...
class hw5Server : public jsonrpc::AbstractServer<hw5Server>
{
    public:
        hw5Server(jsonrpc::AbstractServerConnector &conn, jsonrpc::serverVersion_t type = jsonrpc::JSONRPC_SERVER_V2) : jsonrpc::AbstractServer<hw5Server>(conn, type)
        {
            ...
            this->bindAndAddMethod(jsonrpc::Procedure("end", jsonrpc::PARAMS_BY_NAME, jsonrpc::JSON_OBJECT,  NULL), &hw5Server::endI);
        }
        ...
        inline virtual void endI(const Json::Value &request, Json::Value &response)
        {
            (void)request;
            response = this->end();
        }
